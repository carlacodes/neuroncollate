...
f={}
blockData={}
blocksOfInterest=list(range(187, 266))

for i in range(187, 266):

    user_input = 'D:/Electrophysiological Data/F1704_FloJo/HP_BlockNellie-'+str(i)+'/targetword/orderingbyLRtime/completemiss2s'
...
fig.suptitle('Original Data (all lick releases May-July 2021 FloJo) ', fontsize=10, color='0', y='1')

plt.show() #original data

fig, axes=rasters(cropped_data2,sorted_array, subplots=(4, 8), style='white');
fig.suptitle('Original Data Reorganised (Correct Intra-Trial F0 Roved Responses May-July 2021 FloJo) ', fontsize=10, color='0', y='1')

plt.show() #original data

fig, axes=rasters(shift_aligned_data, sorted_array, subplots=(5, 8),style='white');
fig.suptitle(' Rasters after Shift Model (CORRECT PITCH SHIFT lick releases  May-July /2021 FloJo) ', fontsize=10, color='0', y='1')
...
fig.suptitle(' Rasters after Linear Model (CORRECT PITCH SHIFT lick releases  May-July /2021 FloJo) ', fontsize=10, color='0', y='1')
...
fig.suptitle(' Rasters after Linear Model (ordered by LR onset 24-28/05/2021 FloJo) ', fontsize=10, color='0', y='1')
...
BASE_PATH='D:/Electrophysiological Data/F1704_FloJo/dynamictimewarping/batchcompletemiss'
...
np.save(os.path.join(BASE_PATH, 'mayjuly21neuronIDs'), shift_aligned_data["neurons"])
np.save(os.path.join(BASE_PATH, 'mayjuly21trialIDs'), shift_aligned_data["trials"])

file_name='alignedDataBlockweekmayjuly212021LinearModellickrelease'
np.save(os.path.join(BASE_PATH, file_name), linear_aligned_data["spiketimes"])
np.save(os.path.join(BASE_PATH, 'mayjuly21linearModelneuronIDs'), linear_aligned_data["neurons"])
np.save(os.path.join(BASE_PATH, 'mayjuly21linearModeltrialIDs'), linear_aligned_data["trials"])
...
np.save(os.path.join(BASE_PATH, 'mayjuly21OriginalModelneuronIDs'), cropped_data2["neurons"])
np.save(os.path.join(BASE_PATH, 'mayjuly21OriginalModeltrialIDs'), cropped_data2["trials"])