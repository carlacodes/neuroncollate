...
for i in blocksOfInterest:
    user_input = 'D:/Electrophysiological Data/F1704_FloJo/HP_BlockNellie-' + str(
        i) + '/targetword/soundOnset/orderingbyLRtime/nomisses2s'
...
# objMat=[]
for i in blocksOfInterest:
    user_input = 'D:/Electrophysiological Data/F1704_FloJo/LFP_BlockNellieL22/weekjuly192021//trialonset//LFP_Block' + str(
        i)
    directory = os.listdir(user_input)
...
for i2 in range(len(blocksOfInterest) - 1):
    if i2 == 0:
        adjustedTrial[i2] = blockData[blocksOfInterest[i2 + 1]]["oneDtrialIDarray"] + max(
            blockData[blocksOfInterest[i2]]["oneDtrialIDarray"])
    else:
...
for i3 in range(len(blockDataLFP) - 1):
    ...
    if i3 == 0:
        ...
        combinedLFPtrials3 = np.concatenate((combinedLFPtrials3, np.array(objMatBlock[blocksOfInterest[i3 + 1]])),
                                            axis=1)
        combinedTimetrials3 = np.concatenate((combinedTimetrials3, np.array(objTimeBlock[blocksOfInterest[i3 + 1]])),
                                             axis=1)
...
pop_mean2 = cropped_data2.bin_spikes(int(TMAX - TMIN)).mean(axis=2) / (BINSIZE * 1e-3)
tx2 = np.linspace(TMIN, TMAX, int(TMAX - TMIN))
...
hist -= hist.mean(axis=0, keepdims=True)
hist /= hist.std(axis=0, keepdims=True)
...
hist2 -= hist2.mean(axis=0, keepdims=True)
hist2 /= hist2.std(axis=0, keepdims=True)



plt.scatter(times[idx], trials[idx],s=10, c='k')
...
tx = np.linspace(TMIN, TMAX, 50)
plt.plot(tx, hist, 'black')
...
plt.title('PSTH Plot for Site 26 (Control F0 Trials)')
plt.xlabel('Time Relative to Target Onset (ms)')
plt.ylabel('Lick Release Time (ms)')
plt.show();
...
plt.xticks(np.arange(math.floor(200), math.ceil(2200), math.ceil(100)), np.arange(math.floor(0), math.ceil(2000), math.ceil(100)))
...
lagBtSignals = scipy.signal.signaltools.correlate(hist2, pop_meanLFP)
dt = np.arange(1 - int(TMAX - TMIN), int(TMAX - TMIN))
recovered_time_shift = dt[lagBtSignals.argmax()]
plt.plot(lagBtSignals)
plt.title(['Cross-Correlation between TDT', str(26), ' (WARP 11) and LFP of TDT 7 (WARP 3)'])
...
f, t, Sxx =scipy.signal.spectrogram(hist2, fs=1000)
plt.pcolormesh(t, f, Sxx, shading='gouraud')
plt.ylim([0 ,20])
