...
f={}
blockData={}
blocksOfInterest=[6,7,9, 10,11]
for i in blocksOfInterest:
    user_input = 'D:/Electrophysiological Data/F1815_Cruella/HP_BlockNellie-'+str(i)+'/targetword/soundOnset/orderingbyLRtime/withmisses2sBB2BB3'
...
TMAX =0.5*1000#max(combinedLickReleaseTimes) # s
BINSIZE = 0.01*1000  # 10 ms
NBINS = int((TMAX - TMIN) / BINSIZE)
TMINz=0.2*1000;
...
for i in range(0,len(neuronsbychan)):
    ...
    # print(counter)
    if selectedmeans<0:
        if (selectedmeans)<=(2*bigMean): #selectedmeans-counter: #-(0.1*(counter)):
            print('something good')
            goodChanlist=np.append(goodChanlist, int(key))
    else:
        if (selectedmeans) >= abs(1.5*bigMean):  # selectedmeans-counter: #-(0.1*(counter)):
            print('something good')
            goodChanlist = np.append(goodChanlist, int(key))
...
fig.suptitle('Original Data (all lick releases 07/02/2022 Cruella) ', fontsize=10, color='0', y='1')

plt.show() #original data

fig, axes=rasters(cropped_data2,sorted_array, subplots=(5, 8), style='white');
fig.suptitle('Original Data Reorganised (CORRECT releases 07/02/2022 Cruella) ', fontsize=10, color='0', y='1')

plt.show() #original data

fig, axes=rasters(shift_aligned_data, sorted_array, subplots=(5, 8),style='white');
fig.suptitle(' Rasters after Shift Model (CORRECT releases  07/02/2022 Cruella) ', fontsize=10, color='0', y='1')
...
fig.suptitle(' Rasters after Linear Model (CORRECT releases  07/02/2022 Cruella) ', fontsize=10, color='0', y='1')
...
fig.suptitle(' Rasters after Linear Model (ordered by LR onset 07/02/2022 Cruella) ', fontsize=10, color='0', y='1')
...
BASE_PATH='D:/Electrophysiological Data/F1815_Cruella/dynamictimewarping/soundOnset/withLRmetadata'
...
# np.save(os.path.join(BASE_PATH, 'june0721neuronIDsnPS'), shift_aligned_data["neurons"])
# np.save(os.path.join(BASE_PATH, 'june0721trialIDsnPS'), shift_aligned_data["trials"])
#
...
# np.save(os.path.join(BASE_PATH, 'june0721linearModelneuronIDsnPS'), linear_aligned_data["neurons"])
# np.save(os.path.join(BASE_PATH, 'june0721linearModeltrialIDsnPS'), linear_aligned_data["trials"])
