...
for k00 in pitch_shift_or_not:
    ...
    blocksOfInterest = [25,26,28,30,32,33,34,]
    ...
    for k0 in left_hand_or_right:
        ...
        file_name='alignedDataBlockweekoctintratrial2022ShiftModellickrelease'

        np.save(os.path.join(BASE_PATH2, file_name), shift_aligned_data["spiketimes"])
        np.save(os.path.join(BASE_PATH2, 'octintratrial2022neuronIDsPS'), shift_aligned_data["neurons"])
        np.save(os.path.join(BASE_PATH2, 'octintratrial2022trialIDsPS'), shift_aligned_data["trials"])
        file_name='alignedDataBlockweekoctintratrial2022LinearModellickrelease'

        file_name = 'alignedDataBlockweekoctintratrial2022LinearModellickrelease'
        np.save(os.path.join(BASE_PATH2, file_name), linear_aligned_data["spiketimes"])
        np.save(os.path.join(BASE_PATH2, 'octintratrial2022linearModelneuronIDsPS'), linear_aligned_data["neurons"])
        np.save(os.path.join(BASE_PATH2, 'octintratrial2022linearModeltrialIDsPS'), linear_aligned_data["trials"])
        file_name='alignedDataBlockweekoctintratrial2022OriginalModellickrelease'

        file_name = 'alignedDataBlockweekoctintratrial2022OriginalModellickrelease'
        np.save(os.path.join(BASE_PATH2, file_name), cropped_data2["spiketimes"])
        np.save(os.path.join(BASE_PATH2, 'octintratrial2022OriginalModelneuronIDsPS'), cropped_data2["neurons"])
        np.save(os.path.join(BASE_PATH2, 'octintratrial2022OriginalModeltrialIDsPS'), cropped_data2["trials"])
