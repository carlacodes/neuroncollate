...
fig.suptitle('Original Data Aligned to Distractor (all lick releases 17/05/2021 Zola) ', fontsize=10, color='0', y='1')

plt.show() #original data

fig, axes=rasters(cropped_data2,sorted_array, subplots=(5, 8), style='white');
fig.suptitle('Original Data Ordered, Aligned to Distractor (CORRECT non Pitch Shift lick releases 17/05/2021 Zola) ', fontsize=10, color='0', y='1')

plt.show() #original data

fig, axes=rasters(shift_aligned_data, sorted_array, subplots=(5, 8),style='white');
fig.suptitle(' Rasters after Shift Model (CORRECT  non Pitch Shift  lick releases  17/05/2021 Zola) ', fontsize=10, color='0', y='1')
...
fig.suptitle(' Rasters after Linear Model (CORRECT non Pitch Shift lick releases  17/05/2021 Zola) ', fontsize=10, color='0', y='1')
...
fig.suptitle(' Rasters after Linear Model (ordered by LR onset week 17/05/2021 Zola) ', fontsize=10, color='0', y='1')
...
BASE_PATH='D:/Electrophysiological Data/F1702_Zola_Nellie/dynamictimewarping/comdistnoPitchShift/may172021'
#os.mkdir(BASE_PATH)
file_name='alignedDataBlockweekmay172021ShiftModellickreleaseDist'
np.save(os.path.join(BASE_PATH, file_name), shift_aligned_data["spiketimes"])
np.save(os.path.join(BASE_PATH, 'may17neuronIDsnPS'), shift_aligned_data["neurons"])
np.save(os.path.join(BASE_PATH, 'may17trialIDsnPS'), shift_aligned_data["trials"])

file_name='alignedDataBlockweekmay172021LinearModellickreleaseDist'
...
file_name='alignedDataBlockweekmay172021OriginalModellickreleaseDist'
np.save(os.path.join(BASE_PATH, file_name), cropped_data2["spiketimes"])
np.save(os.path.join(BASE_PATH, 'may17OriginalModelneuronIDsnPS'), linear_aligned_data["neurons"])
np.save(os.path.join(BASE_PATH, 'may17OriginalModeltrialIDsnPS'), linear_aligned_data["trials"])
