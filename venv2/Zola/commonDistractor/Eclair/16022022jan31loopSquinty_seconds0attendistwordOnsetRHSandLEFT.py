...
list_of_distractors=[2,3,4]
meaning_of_word=['sailor', 'accurate' , 'but in']
    ...
    for k0 in pitch_shift_option:
        ...
        for i0 in list_of_distractors:
            ...
            for i in blocksOfInterest:
                user_input = 'D:/Electrophysiological_Data/F1604_Squinty/HP_BlockNellie-' + str(
            ...
            plt.show();
            #
            # BASE_PATH='D:/Electrophysiological Data/F1604_Squinty/dynamictimewarping/soundOnset/withLRmetadata'
            ...
            BASE_PATH = 'D:/Electrophysiological Data/F1604_Squinty/dynamictimewarping/'
            file_name = 'alignedDataBlockweekdecember142022ShiftModellickrelease'
            if os.path.isdir(BASE_PATH) is False:
                os.mkdir(BASE_PATH)
            BASE_PATH2 = 'D:/Electrophysiological Data/F1604_Squinty/dynamictimewarping/l22' + k0 + 'distF' + str(
                i0) + '/' + k00 + '/'
            ...
            np.save(os.path.join(BASE_PATH2, 'december142022neuronIDsPS'), shift_aligned_data["neurons"])
            np.save(os.path.join(BASE_PATH2, 'december142022trialIDsPS'), shift_aligned_data["trials"])

            file_name = 'alignedDataBlockweekdecember142022LinearModellickrelease'
            np.save(os.path.join(BASE_PATH2, file_name), linear_aligned_data["spiketimes"])
            np.save(os.path.join(BASE_PATH2, 'december142022linearModelneuronIDsPS'), linear_aligned_data["neurons"])
            np.save(os.path.join(BASE_PATH2, 'december142022linearModeltrialIDsPS'), linear_aligned_data["trials"])

            file_name = 'alignedDataBlockweekdecember142022OriginalModellickrelease'

            np.save(os.path.join(BASE_PATH2, file_name), cropped_data2["spiketimes"])
            np.save(os.path.join(BASE_PATH2, 'december142022OriginalModelneuronIDsPS'), cropped_data2["neurons"])
            np.save(os.path.join(BASE_PATH2, 'december142022OriginalModeltrialIDsPS'), cropped_data2["trials"])