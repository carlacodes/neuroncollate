...
fig.suptitle('Original Data Aligned to Distractor (all lick releases 21/06/2021 Zola) ', fontsize=10, color='0', y='1')

plt.show() #original data

fig, axes=rasters(cropped_data2,sorted_array, subplots=(5, 8), style='white');
fig.suptitle('Original Data Ordered, Aligned to Distractor (CORRECT non Pitch Shift lick releases 21/06/2021 Zola) ', fontsize=10, color='0', y='1')

plt.show() #original data

fig, axes=rasters(shift_aligned_data, sorted_array, subplots=(5, 8),style='white');
fig.suptitle(' Rasters after Shift Model (CORRECT  non Pitch Shift  lick releases  21/06/2021 Zola) ', fontsize=10, color='0', y='1')
...
fig.suptitle(' Rasters after Linear Model (CORRECT non Pitch Shift lick releases  21/06/2021 Zola) ', fontsize=10, color='0', y='1')
...
fig.suptitle(' Rasters after Linear Model (ordered by LR onset week 21/06/2021 Zola) ', fontsize=10, color='0', y='1')
...
BASE_PATH='D:/Electrophysiological Data/F1702_Zola_Nellie/dynamictimewarping/comdistnoPitchShift/june212021'
#os.mkdir(BASE_PATH)
file_name='alignedDataBlockweekjune212021ShiftModellickreleaseDist'
np.save(os.path.join(BASE_PATH, file_name), shift_aligned_data["spiketimes"])
np.save(os.path.join(BASE_PATH, 'june21neuronIDsnPS'), shift_aligned_data["neurons"])
np.save(os.path.join(BASE_PATH, 'june21trialIDsnPS'), shift_aligned_data["trials"])

file_name='alignedDataBlockweekjune212021LinearModellickreleaseDist'
np.save(os.path.join(BASE_PATH, file_name), linear_aligned_data["spiketimes"])
np.save(os.path.join(BASE_PATH, 'june21linearModelneuronIDsnPS'), linear_aligned_data["neurons"])
np.save(os.path.join(BASE_PATH, 'june21linearModeltrialIDsnPS'), linear_aligned_data["trials"])

file_name='alignedDataBlockweekjune212021OriginalModellickreleaseDist'
np.save(os.path.join(BASE_PATH, file_name), cropped_data2["spiketimes"])
np.save(os.path.join(BASE_PATH, 'june21OriginalModelneuronIDsnPS'), linear_aligned_data["neurons"])
np.save(os.path.join(BASE_PATH, 'june21OriginalModeltrialIDsnPS'), linear_aligned_data["trials"])