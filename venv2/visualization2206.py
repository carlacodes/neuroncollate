...
def rasters(data,sorted_array, subplots=(5, 6), fig=None, axes=None, figsize=(9*1.5, 5*1.5),
    ...
    **scatter_kw
    ...
    for n, ax in enumerate(axes.ravel()):
        ...
        ax.set_xticks(np.arange(math.floor(min(times)), math.ceil(max(times)), math.ceil(max(times))/2))
        ...
        ax.set_yticks(np.arange(math.floor(min(sorted_array[:, 1])), math.ceil(max(sorted_array[:, 1])),
                               math.ceil(max(sorted_array[:, 1] / 3))))
        ...
        if min(sorted_array[:,0])<0:
            ax.set_yticklabels(np.arange(math.floor(min(sorted_array[:,0])), math.floor(max(sorted_array[:,0])), math.ceil((max(sorted_array[:,0])-min(sorted_array[:,0]))/3)))
        elif max(sorted_array[:, 0]) == 0.0:
            ax.set_yticklabels(np.arange(math.floor(min(sorted_array[:,1])), math.ceil(max(sorted_array[:,1])), math.ceil(max(sorted_array[:,1])/3)))

        else:
            ax.set_yticklabels(np.arange(math.floor(min(sorted_array[:,0])), math.ceil(max(sorted_array[:,0])), math.ceil(max(sorted_array[:,0])/3)))
        ax.set_xlabel('milliseconds')
